...
const SingleItem = () => {
  ...
  const handlePopupForm = () => {
    setEditedItem({
      ...
      productCategory: item.productCategory?.product_category_name || '',
      measuring_unit: item.measuring_unit || '',
      low_limit: item.low_limit?.toString() || '', // Convert to string for input field
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const updatedItem = {
        ...
        product_category_name: editedItem.productCategory,
      ...
      // Update the item state based on the response structure
      if (response.data.updatedItem) {
        setItem(response.data.updatedItem);
      } else if (response.data) {
        setItem(response.data);
      } else {
        // If server doesn't return updated item, update state manually
        setItem(prevItem => ({
          ...prevItem,
          ...updatedItem
        }));
          ...
          <div className="flex px-11 justify-between mt-7 w-9/12">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex gap-4">
                ...
                <span className="font-medium">{item.productCategory?.product_category_name}</span>
              </div>
              <div className="flex gap-4">
                <p className="font-semibold">Item Category:</p>
                <span className="font-medium">{item.itemCategory?.item_category_name}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div className="flex justify-between items-center gap-9">
              ...
              <input
                ...
                className="border-2 border-gray p-1 pl-3 rounded-md w-64"
                value={editedItem.productCategory}
                onChange={handleChange}
              />
            </div>
            <div className="flex justify-between items-center">
